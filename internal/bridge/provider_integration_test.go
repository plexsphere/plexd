...
// TestProviderIntegration_ConcurrentAccessWithProvider exercises concurrent SSE
// events (config updates, peer assignments, peer revocations) alongside the
// reconcile loop with a provider active. Run with -race.
func TestProviderIntegration_ConcurrentAccessWithProvider(t *testing.T) {
	ctrl := &mockAccessController{}
	routes := &mockRouteController{}
	provider := newMockUserAccessProvider("tailscale")
	cfg := providerTestConfig()
	...
	// Peer assignment dispatchers — use unique keys per goroutine.
	for i := 0; i < 10; i++ {
		wg.Add(1)
		go func(idx int) {
			defer wg.Done()
	...
	// Concurrent status reads to exercise provider status reporting under load.
	for i := 0; i < 10; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
			_ = mgr.UserAccessStatus()
		}()
	}
	...
	// Provider should still be stable (started once, not stopped).
	if startCount := providerCallCount(provider, "Start"); startCount != 1 {
		t.Errorf("provider.Start calls = %d, want 1", startCount)
	}
	if stopCount := providerCallCount(provider, "Stop"); stopCount != 0 {
		t.Errorf("provider.Stop calls = %d, want 0", stopCount)
	}
}
