...
// To disable audit forwarding, set Enabled=false before or after calling ApplyDefaults.
...
func (c *Config) Validate() error {
	...
	if c.CollectInterval < time.Second {
		return errors.New("auditfwd: config: CollectInterval must be at least 1s")
	}
	if c.ReportInterval < c.CollectInterval {
		return errors.New("auditfwd: config: ReportInterval must be >= CollectInterval")
	}
	if c.BatchSize < 1 {
